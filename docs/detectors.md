...
`integrity → metadata → visual → static → dup_frame → jump → endpoint_static → freeze → noise → brightness → gripper_offscreen → regrasp → object_slip → colormatch → tablecloth`。
除 `gripper_offscreen`（规范12）/ `regrasp`（规范1）/ `object_slip`（规范21）/ `colormatch`（规范19）/ `tablecloth`（规范17）五项付费多模态远程调用**默认关闭**外，其余默认全开。
...
### 3ter.5 TableclothChecker（误夹桌布，规范序号 17）
