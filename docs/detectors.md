...
3. 归约：按 side(left/right/single) 各建两条序列——hold_seq[i]=持有标签或 None；
   closed_seq[i]=夹爪是否闭合(True/False/None=未知)。
...
- 物体小 / 与夹爪同色 / 桌面拥挤时逐帧「是否持有 / 夹爪是否闭合」会不准。
...
| object_slip | WARN（默认关闭） | 持有结束时夹爪仍闭合却脱手（滑落） | ffmpeg + google-genai 或 openai + 网络 | 缺 key/SDK、抽帧/接口异常 → WARN |
