...
- **质检规范专项检测器**：
  ...
  - `frame_consistency`：画面与关节一致性（规范18）。腕部相机（`camera_left/right`）随对应臂运动，
    用 LeRobot parquet 关节作"该臂动没动"的地面真值、相邻帧帧差作"画面动没动"，臂在动而腕部相机
    画面贴自身底噪的连续段 > 1s 即命中（采集冻结/串流延迟/掉帧/贴错相机）。无需 AI；「某臂确实没动」
    天然不误报；非腕部机位（`camera_top`）/非 LeRobot 视频跳过，缺 parquet/pyarrow 降级 WARN
...
- **多模态检测器（默认关闭，可插拔后端）**：
  ...
  - `occlusion`：首帧夹爪遮挡操作物品（规范15）。静态属性、无需时序——只抽视频**开头一小段**
    （`head_sec` 默认 1s，忠实「首帧」语义、省成本）→模型逐帧判「被操作物体是否被夹爪/机械臂遮挡」
    →代码侧在可识别物体的首段帧里按「被遮挡」占比 ≥ 阈值判定。`provider=gemini|openai` 可切换。
  - `edge_grasp`：夹取位置过于极限（规范16）。静态属性、逐帧独立判——本地抽帧→模型逐帧判
    「夹爪夹的是物体边缘/棱角还是主体/重心」（只对看清夹住物体的帧返回）→代码侧在可判定的帧里
    按「夹在边缘」占比 ≥ 阈值判定。`provider=gemini|openai` 可切换。
  - `tablecloth`：误夹桌布（规范17）。含桌布场景专项——本地抽帧→模型逐帧判「该帧是否含桌布」
    及「夹爪是否连带桌布被夹起/牵拉变形」→代码侧先门控（含桌布帧不足则本项不适用判 PASS），
    再在含桌布帧里按「误夹」占比 ≥ 阈值且命中帧数达标判定。`provider=gemini|openai` 可切换。
